...
import { X, ArrowUpRight, ShieldCheck, Check, AlertCircle, Lock, Wallet } from 'lucide-react';
      ...
      <div className="bg-[#0E131F] border border-border w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-border flex items-center justify-between bg-surface/50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Retirer des Fonds</h3>
              <p className="text-xs text-gray-400">Solde disponible : {formatCurrency(balance, currency)}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-4">
          ...
          <form onSubmit={handleWithdrawal} className="space-y-4">
            ...
            <div>
              <label className="text-xs font-semibold text-gray-300 uppercase block mb-1.5 font-mono">
                {isFcfa ? 'Numéro Mobile Money (Wave / Orange / MTN)' : 'Compte de destination (IBAN / Adresse Crypto)'}
              </label>
              <input
                type="text"
                required
                ...
                disabled={!isEmailVerified}
                className="w-full bg-card border border-border focus:border-primary rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none transition disabled:opacity-50 font-mono"
              />
            </div>
            ...
            <button
              ...
              className="w-full mt-2 py-3.5 rounded-xl bg-primary text-black font-black text-sm uppercase tracking-wider hover:bg-primary-hover active:scale-98 transition flex items-center justify-center gap-2 shadow-lg shadow-primary/20 disabled:opacity-40"
            >
              {isLoading ? (
                <div className="w-5 h-5 rounded-full border-2 border-black/30 border-t-black animate-spin" />
              ) : (
                <>
                  <span>Confirmer le Retrait de {formatCurrency(amount, currency)}</span>
